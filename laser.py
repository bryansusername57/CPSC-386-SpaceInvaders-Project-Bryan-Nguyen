...
class Lasers:
    ...
    def fire(self): 
        if type(self.owner) is alien.AlienFleet:
            new_laser = Laser(self.game, owner=self.alien_fleet)
    ...
    def update(self):
        ...
        if type(self.owner) is self.alien_fleet: 
            if laser.rect.top >= self.screen.bottom:
                self.lasers.remove(laser)

        collisions = pg.sprite.groupcollide(self.alien_fleet.fleet, self.lasers, False, True)
        #ship_collisions = pg.sprite.groupcollide(self.Ship.ship, self.alien_lasers, False, True)
